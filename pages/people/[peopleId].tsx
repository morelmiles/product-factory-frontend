...
const ProfileList: React.FunctionComponent = ({ peopleId }) => {
  const { data, error, loading } = useQuery(GET_PROFILE, {
    variables: { personSlug: peopleId }
  });
  const {
    data: reviews,
    error: reviewError,
                    ...
                    return (
                      <div key={`review-${idx}`} className="grey-border p-24 mb-24">
                        <Row>
                          <Col span={18}>
                            <Link
                              href={`${match.url}/profiles/${getProp(review, 'id', '')}`}
...
ProfileList.getInitialProps = async ({ query }) => {
    const { peopleId } = query;
    return { peopleId };
}
